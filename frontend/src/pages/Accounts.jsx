...
export default function Accounts() {
  ...
  const handleRemove = async (id) => {
    if (!confirm('Are you sure you want to remove this account? All associated stats will be deleted.')) return;
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="card">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="label">UrNetwork Username (Email/Phone)</label>
              <input type="text" className="input" value={username} onChange={e => setUsername(e.target.value)} required />
            </div>
            <div>
              <label className="label">UrNetwork Password</label>
              <input type="password" className="input" value={password} onChange={e => setPassword(e.target.value)} required />
            </div>
          </div>
          <div>
            <label className="label">Nickname (Optional)</label>
            <input type="text" className="input" value={nickname} onChange={e => setNickname(e.target.value)} placeholder="e.g. Home Network" />
          </div>
      ...
      <div className="card">
        <h2 className="text-xl font-bold mb-4">Current Accounts</h2>
              ...
              {accounts.map(acc => (
                <tr key={acc.id} className="border-b border-[var(--color-border)] hover:bg-[#1e2124] transition-colors">
                  ...
                  <td className="py-3 px-4 space-x-2">
                    <button className="btn btn-secondary text-xs" onClick={() => handleToggle(acc.id)}>Toggle</button>
                    <button className="btn btn-danger text-xs" onClick={() => handleRemove(acc.id)}>Remove</button>
                  </td>
                </tr>
              ))}
              {accounts.length === 0 && (
                <tr>
                  <td colSpan="4" className="py-6 text-center text-[var(--color-text-muted)]">No accounts added yet.</td>
                </tr>
              )}
            </tbody>
          </table>
